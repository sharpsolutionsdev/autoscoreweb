# APK Refinement Design — 2026-04-15
...
APK launch (main.py)
  → loads apk-gate.html
    → Supabase session check + dartvoice_subscriptions lookup
      → no sub      → trial signup CTA (loads /login?source=apk)
      → active sub  → window.location = web-app-mobile.html
  → DartVoiceBridge (JS interface on control WebView)
  → Scorer WebView (lazy, loads app.dartcounter.net)
```

Three JS modules inside web-app-mobile.html:

- **VoiceController** — state machine: IDLE → LISTENING → CHECKOUT → COOLDOWN → RECOVERING
- **ScoreValidator** — rejects invalid dart totals (179/178/176/175/173)
- **ScorerObserver** — via DartVoiceBridge; polls injected DartCounter DOM for
  checkout detection + input keyboard suppression

## Subscription Gate (apk-gate.html)
- Loads Supabase client + existing branding
- On boot: `sb.auth.getSession()` → if no user, show "Sign in / Start Free Trial" CTA
- If user: query `dartvoice_subscriptions.status` — accept `active`, `trialing`
- Pass → `window.location.replace('/web-app-mobile.html')`
- Fail → show gate screen with two buttons: `Sign in` (loads `/login?source=apk`) and
...
States:
- `IDLE` — mic off
- `LISTENING` — normal scoring; full vocabulary
- `CHECKOUT` — active after detected checkout prompt; restricted vocabulary
- `COOLDOWN` — 1500ms lock after successful inject to prevent dupes
- `RECOVERING` — restart SpeechRecognition after error or 8s silence watchdog
...
(reject 173, 175, 176, 178, 179 — impossible three-dart totals).
...
- Injection uses `setReactValue` + `input/change` events (already in DARTCOUNTER_DRIVER);
  no `.focus()` call — that's what triggers the keyboard
...
- Red "Mic lost — reconnecting" during RECOVERING
...
## Tablet Responsive
Breakpoints:
- `<600px` — current mobile layout
- `600-899px` — container max-width 700px, larger mic button, 4-col stat grid
- `≥900px` — side-by-side layout: controls left 420px, activity log right
- Larger font sizes via `clamp()` on display headings

## Files Changed
- NEW: `apk-gate.html` — sub gate screen
- MODIFIED: `web-app-mobile.html` — voice state machine + UI + tablet CSS + mic select
- MODIFIED: `autoscore/main.py` — load `apk-gate.html` first
- MODIFIED: `autoscore/src/com/dartvoice/DartVoiceBridge.java` — status pill,
  keyboard suppression injection, checkout detection bridge
- NEW: `autoscore/res/drawable/` — new icon assets (handled at APK packaging time)
