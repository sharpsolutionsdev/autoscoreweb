...
The core pages (`index.html`, `dartvoice-dashboard.html`, `guide.html`) utilize deep dark modes, premium red accents (`#CC0B20`), heavy glassmorphism, and modern micro-animations to communicate a premium "software" feel before the user even downloads an app.
...
- These links are embedded directly into the CTA buttons on `index.html`.
- Upon successful payment, Stripe handles the automatic recurring billing (SaaS model), and redirects the user to the `thanks.html` success page.
- Canceling a checkout redirects back to `checkout-cancelled.html`.
