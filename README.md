...
## ðŸ— Core Architecture & Repositories
...
- **Key Files:**
  - `html/index.html`: The high-converting marketing landing page featuring 3D hover states, pseudo-software CSS mockups, and Stripe checkout funnels.
  - `html/dartvoice-dashboard.html`: The authenticated portal where active subscribers download the respective clients.
  - `html/guide.html`: A master documentation page detailing Software Installation, First Launch, X01 Calibration, Cricket Calibration, and Voice Commands.
...
- **Payments:** Handled entirely out-of-ecosystem by Stripe (`buy.stripe.com` links are embedded in `html/index.html` and `html/guide.html`).
...
## ðŸš€ Immediate Next Steps & Deployment Checklist

The UI/UX is functionally "Launch Ready." Focus should shift to deployment, backend integration, and store compliance.

### 1. Chrome Extension Submission âœ…
- âœ… Icons generated (16x16, 48x48, 128x128) and wired into `manifest.json`.
- âœ… Extension successfully published to the [Chrome Web Store](https://chromewebstore.google.com/detail/dartvoice-launchpad/igldnjophdpofihidpbblchgfamncpgb).
- âœ… Updated `html/dartvoice-dashboard.html` and `html/guide.html` with the official store link for direct, managed installation.

### 2. Backend API Verification â¬œ
*Note: The frontend is currently static HTML. To make the dashboard and OTP logic functional:*
- â¬œ Ensure the backend server securely validates Stripe Webhooks (e.g. `customer.subscription.created`) and updates the user database.
- â¬œ Connect the static `html/login.html` and `html/dartvoice-dashboard.html` files to an actual routing framework (like Next.js) OR wire up Vanilla JS to consume backend JWT/Session tokens.
- âœ… Lifecycle emails dispatched via Resend API through Supabase Edge Functions (`send-dartvoice-email`, `send-confirmation`). OTP emails sent via Supabase Auth SMTP â†’ Resend. All sent from `@dartvoice.app` domain.

### 3. Windows & Android Builds â¬œ
...
### 4. SEO & Analytics âœ… (Placeholder)
- âœ… Google Analytics `gtag.js` snippet injected into ALL 8 public-facing pages.
- â¬œ **ACTION REQUIRED:** Find-and-replace `G-XXXXXXXXXX` with your real Google Analytics Measurement ID across all HTML files. You can do this with a single command:
  ```
  ...
  # then open http://127.0.0.1:8000/html/dartvoice-dashboard.html
  ```

  - Supabase keys are centralized in `js/config.js` for easier management. For local development, you can create `js/config.local.js` and set `window.SUPABASE_URL` / `window.SUPABASE_KEY` there to override defaults. Do not commit secrets â€” rotate keys if you remove them from the repo.
